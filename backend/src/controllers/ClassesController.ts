...
import convertHourToMin from '../utils/convert';
...
export default class ClassController {
    async create(request: Request, response: Response) {
        ...
        try {
            ...
            const insertUsersIds = await trx('users').insert(user);
            ...
            return response.status(201).send('Posted!');
            ...
            return response.status(400).json(
                {
                    error: "Something went wrong while tried to create a new entity."
                }
            )     
        }
